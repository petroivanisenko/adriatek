            ...
            <div className="lg:col-span-8 space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <Alert className="border-primary/20 bg-primary/5 rounded-2xl p-6">
                  ...
                  <div className="ml-4">
                    <AlertTitle className="text-lg font-bold mb-1">Secure Online Payment</AlertTitle>
                    <AlertDescription className="text-muted-foreground">
                      We use Stripe to process your payment securely. All major cards accepted.
                    </AlertDescription>
                  </div>
                </Alert>
                  ...
                  <form id="checkout-form" onSubmit={handleSubmit} className="space-y-10">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      <div className="space-y-3">
                        ...
                        <Input id="fullName" name="fullName" value={formData.fullName} onChange={handleInputChange} required placeholder="Individual or Company Name" className="h-14 rounded-xl border-primary/10 focus:border-primary transition-all text-lg" />
                      </div>
                      <div className="space-y-3">
                        <Label htmlFor="phone" className="text-sm font-bold uppercase tracking-widest opacity-60">Contact Phone</Label>
                        <Input id="phone" name="phone" type="tel" value={formData.phone} onChange={handleInputChange} required maxLength={16} placeholder="+XX XXX XXX XXXX" className="h-14 rounded-xl border-primary/10 focus:border-primary transition-all text-lg" />
                      </div>
                      <div className="md:col-span-2 space-y-3">
                        ...
                        <div className="relative">
                          ...
                          <Input id="email" name="email" type="email" value={formData.email} onChange={handleInputChange} required placeholder="invoice@company.com" className="h-14 pl-12 rounded-xl border-primary/10 focus:border-primary transition-all text-lg" />
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-8">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                          ...
                          <Select onValueChange={handleCountryChange} required>
                            ...
                            <SelectContent className="max-h-80 rounded-xl">
                              {EUROPEAN_COUNTRIES.map((country) => (
                                <SelectItem key={country} value={country} className="text-lg py-3">{country}</SelectItem>
                              ))}
                            </SelectContent>
                          </Select>
                        </div>
                        <div className="space-y-3">
                          ...
                          <Input id="city" name="city" value={formData.city} onChange={handleInputChange} required placeholder="Metropolitan Area" className="h-14 rounded-xl border-primary/10 focus:border-primary transition-all text-lg" />
                        </div>
                        <div className="md:col-span-2 space-y-3">
                          <Label htmlFor="address" className="text-sm font-bold uppercase tracking-widest opacity-60">Street Operations Address</Label>
                          <Input id="address" name="address" value={formData.address} onChange={handleInputChange} required placeholder="Full address details for courier delivery" className="h-14 rounded-xl border-primary/10 focus:border-primary transition-all text-lg" />
                        </div>
                        <div className="space-y-3">
                          <Label htmlFor="postalCode" className="text-sm font-bold uppercase tracking-widest opacity-60">Postal Index</Label>
                          <Input id="postalCode" name="postalCode" value={formData.postalCode} onChange={handleInputChange} required placeholder="HQ/Office Code" className="h-14 rounded-xl border-primary/10 focus:border-primary transition-all text-lg" />
                        </div>
                        <div className="space-y-3">
                          ...
                          <Input id="additionalInfo" name="additionalInfo" value={formData.additionalInfo} onChange={handleInputChange} placeholder="Company VAT, specific gate instructions, etc." className="h-14 rounded-xl border-primary/10 focus:border-primary transition-all text-lg" />
                        </div>
                      </div>
                    </div>

                    <Button form="checkout-form" type="submit" className="h-16 px-12 text-xl font-black rounded-2xl shadow-2xl shadow-primary/30 transition-all hover:scale-[1.02] flex-1 w-full" disabled={isSubmitting}>
                      {isSubmitting ? "INITIATING SECURE PAYMENT..." : "PROCEED TO PAYMENT"}
                      ...
                      {cart.items.map((item) => (
                        <div key={item.productId} className="flex gap-4 group">
                          <div className="relative size-16 shrink-0 rounded-xl overflow-hidden bg-white border border-primary/5">
                            <Image src={resolvePublicImageUrl(item.product.image) ?? ""} alt={item.product.name} fill className="object-contain p-2 transition-transform duration-500 group-hover:scale-110" unoptimized={(resolvePublicImageUrl(item.product.image) ?? "").includes("localhost")} />
                          </div>
                          <div className="flex-1 min-w-0 flex flex-col justify-center">
                            <p className="font-bold text-sm line-clamp-2 leading-tight mb-1 group-hover:text-primary transition-colors">{item.product.name}</p>
